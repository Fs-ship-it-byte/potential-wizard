...
- ✅ Manifest + catálogos (populares, mejor valoradas, por género, búsqueda) vía TMDB.
- ✅ Meta de películas vía TMDB.
- ✅ Stream handler: busca el título en las 5 fuentes en paralelo.
- ✅ **Resolución a link directo**: los embeds de la familia Streamwish
  (streamwish, niramirus, filemoon, vidhide, hgplaycdn, etc.) y VidHide/filelions
  se resuelven a `.m3u8` real con una cadena de 3 intentos — axios rápido →
  navegador headless (Puppeteer, capturando la petición de red real) → genérico —
  y se sirven a través de un **proxy HLS propio** (`/hlsproxy/...`) para evitar
  el bloqueo del CDN por IP/Referer. Solo cae a "enlace externo" si los 3
  intentos fallan.
...
| `PUBLIC_URL`   | URL pública del addon (la que te da Railway), sin `/` final | Sí (para que el proxy HLS funcione) |
