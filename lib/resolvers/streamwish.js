...
const EMBED_HOSTS = [
  ...
  'd0000d', 'dooodster',
  // Vimeo (embed público o "privado" con hash de dominio)
  'vimeo.com', 'player.vimeo'
];
...
async function resolveStreamwishHlsViaBrowser(embedUrl, timeoutMs, trace) {
  ...
  try {
    ...
    page.on('request', (req) => {
      ...
      if (!resolved && (/\.m3u8(\?|$)/i.test(url) || /master\.json(\?|$)/i.test(url))) {
        resolved = {
          url,
          headers: {
        ...
        t('¡MATCH! manifiesto capturado (por URL): ' + url);
      }
...
async function resolveToDirectHls(playerOrEmbedUrl) {
  ...
  if (!isPlayerPage && !needsBrowserOnly(playerOrEmbedUrl)) {
