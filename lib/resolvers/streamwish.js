...
async function resolveStreamwishHlsViaBrowser(embedUrl, timeoutMs, trace) {
  ...
  try {
    ...
    // Si el sitio muestra un captcha (hCaptcha/reCAPTCHA/Cloudflare Turnstile),
    // ningún click automatizado lo va a resolver -- es una barrera real que
    // necesita un humano. Detectarlo temprano evita esperar los 25-35s
    // completos sin sentido, y libera el cupo de concurrencia para que otro
    // servidor del mismo título tenga su chance real.
    ...
    page.on('request', (req) => {
      ...
      if (!captchaDetected && /hcaptcha\.com|recaptcha|gstatic\.com\/recaptcha|challenges\.cloudflare\.com/i.test(urlLower)) {
        captchaDetected = url;
        t('CAPTCHA detectado (' + new URL(url).host + ') -- esto no lo puede resolver un click automatizado, cortando temprano');
      }
    ...
    let captchaDetectedAt = null;
    // Le damos un margen corto después de detectar el captcha por si el
    // video en realidad no depende de resolverlo (algunos sitios cargan
    // hCaptcha para otra cosa, como comentarios, sin bloquear el player) --
    // pero si en ese margen no apareció nada, cortamos ya en vez de agotar
    // el timeout completo sin sentido.
    const CAPTCHA_GRACE_MS = 4000;
    while (!resolved && Date.now() - start < timeoutMs) {
      if (captchaDetected && !captchaDetectedAt) captchaDetectedAt = Date.now();
      if (captchaDetectedAt && Date.now() - captchaDetectedAt > CAPTCHA_GRACE_MS) {
        t('Cortando temprano: pasaron ' + CAPTCHA_GRACE_MS + 'ms desde el captcha y no apareció ningún manifiesto');
        break;
      }
    ...
    if (!resolved && captchaDetected) {
      t('No se pudo resolver: el sitio muestra un CAPTCHA (' + captchaDetected + ') que requiere resolución humana.');
    }
    return resolved;
  } catch (e) {
