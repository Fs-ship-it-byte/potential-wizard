...
async function resolveToDirectHls(playerOrEmbedUrl) {
  ...
  // 2a-VOE. VOE ofusca todo del lado del cliente (base64 + JS reversible que
  // cambia seguido) — no vale la pena mantener un decoder frágil a mano.
  // Vamos directo al navegador headless y capturamos la petición de red real
  // al .m3u8/.mp4, igual que con streamwish.
  if (isVoeHost(embedUrl)) {
    const viaBrowserVoe = await resolveStreamwishHlsViaBrowser(embedUrl);
    if (viaBrowserVoe && viaBrowserVoe.url) return viaBrowserVoe;
    // último recurso genérico si el navegador no encontró nada
    const directVoe = await resolveDirectVideoUrl(embedUrl);
    if (directVoe && directVoe.url) return directVoe;
    return null;
  }
