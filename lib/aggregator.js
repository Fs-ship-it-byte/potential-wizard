...
async function resolveEmbedToDirect(stream) {
  ...
  try {
    // Cubre streamwish/niramirus/filemoon/vidhide/hgplaycdn/voe/etc, con la
    // cadena completa: axios rápido -> Puppeteer -> genérico -> vidhide.
    if (SW.isEmbedHost(url) || SW.isVoeHost(url) || SW.isDoodHost(url) || SW.isVimeoHost(url) || /vidhide|filelions|player\.php|player\./i.test(url)) {
...
// Por ahora cubre GNULA y Poseidon HD, que son las 2 fuentes con parser de
// episodio ya portado. PelisGo/Refugio solo tienen metadata de temporadas,
// no un fetcher de episodio individual — quedan pendientes.
async function getEpisodeStreams(title, year, season, episode) {
  const [gn, ps] = await Promise.all([
