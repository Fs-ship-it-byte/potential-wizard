...
const GLOBAL_RESOLVE_DEADLINE_MS = 28000;
...
// Devuelve los resultados en el mismo orden que `items` (como Promise.all).
...
async function getStreams(title, year) {
  ...
  // todos de una) para no saturar el navegador headless compartido. Además,
  // GLOBAL_RESOLVE_DEADLINE_MS es un techo de seguridad: si por tener MUCHOS
  // servidores el total de "rondas" se alarga demasiado, devolvemos lo que
  // haya (los que no llegaron a tiempo, tal cual llegaron -- como enlace
  // externo) en vez de arriesgar que Stremio descarte la respuesta entera.
  all = await withTimeout(
    mapWithConcurrencyLimit(all, MAX_CONCURRENT_BROWSER_RESOLVES, (s) => (
      s.type === 'embed' ? resolveEmbedToDirect(s) : s
    )),
