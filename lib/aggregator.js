...
async function fromPoseidon(title, year) {
  ...
  return data.streams.map((s) => ({
    name: 'Poseidon HD', title: s.label || 'Servidor', url: s.playerUrl, type: 'embed', hostHint: s.locker || null
  }));
}
...
async function getStreams(title, year) {
  ...
  let all = interleave([ps, gn, cc, pg, rf]);

  // Resolvemos los embeds de a MAX_CONCURRENT_BROWSER_RESOLVES por vez (no
  // todos de una) para no saturar el navegador headless compartido.
  // GLOBAL_RESOLVE_DEADLINE_MS es un techo de seguridad para el conjunto: si
  // por tener MUCHOS servidores el total se alarga demasiado, mapWithConcurrencyLimit
  // devuelve lo que haya (los que ya resolvieron con su link directo, los que
  // no llegaron a tiempo tal cual llegaron) -- nunca descarta lo que ya
  // funcionó.
  all = await mapWithConcurrencyLimit(
    all,
    MAX_CONCURRENT_BROWSER_RESOLVES,
    (s) => (s.type === 'embed' ? resolveEmbedToDirect(s) : s),
