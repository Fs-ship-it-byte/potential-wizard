...
// Recibe una lista de títulos válidos (localizado + original) y evalúa cada
// candidato contra TODOS ellos, quedándose con el mejor puntaje. Esto es
// clave: el sitio puede devolver el resultado correcto buscando con un
// título (ej. "House of the Dragon", por fuzzy-search del sitio) pero
// mostrar el título en OTRO idioma (ej. "La casa del dragón") -- si solo
// comparáramos contra el término de búsqueda, ese resultado correcto se
// rechazaría por "no tener ninguna palabra en común" a pesar de ser el show
// correcto. Comparando contra todas las variantes conocidas, lo reconoce.
function bestMatch(candidates, titles, year, getTitle, getYear) {
  const nts = (Array.isArray(titles) ? titles : [titles]).map(normTitle).filter(Boolean);
  ...
  for (const c of candidates) {
    ...
    let score = 0;
    for (const nt of nts) {
      if (ct === nt) score = Math.max(score, 10);
      else if (ct.indexOf(nt) !== -1 || nt.indexOf(ct) !== -1) score = Math.max(score, 5);
    }
    if (score === 0) continue;
...
async function searchBestMatch(searchFn, label, titles, year, getTitle, getYear, filterFn) {
  ...
  for (const t of titles) {
    ...
    // OJO: se le pasan TODAS las variantes de título (no solo `t`, la que se
    // usó para buscar) -- ver comentario en bestMatch() de por qué importa.
    match = bestMatch(candidates.length ? candidates : results, titles, year, getTitle, getYear);
    if (match) break;
  }
...
async function fromFuegocine(title, year, originalTitle) {
  const titles = titleVariants(title, originalTitle);
  const match = await searchBestMatch(FC.searchFc, 'fuegocine', titles, year, (r) => r.titulo, (r) => r.year);
  if (!match) return [];
  const htmlRes = await safe(httpShim.request(FC.fcMobileUrl(match.url), { headers: {}, compression: true, noFail: true }), 'fuegocine-page');
  const html = htmlRes ? htmlRes.toString() : null;
  if (!html) return [];
  const data = FC.parseFcDetail(html);
  if (!data || !data.links) return [];
  return data.links.map((l) => ({
    name: 'FuegoCine', title: [l.name, l.lang, l.quality].filter(Boolean).join(' · ') || 'Servidor', url: l.url, type: 'embed'
  }));
}
...
async function getStreams(title, year, originalTitle) {
  const [cc, gn, pg, rf, fc] = await Promise.all([
    ...
    fromFuegocine(title, year, originalTitle)
  ]);

  let all = interleave([gn, cc, fc, pg, rf]);
